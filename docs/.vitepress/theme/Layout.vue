<script setup>
import DefaultTheme from 'vitepress/theme'
import { onMounted, onUnmounted, nextTick } from 'vue'
import { useRouter } from 'vitepress'
import { gsap } from 'gsap'
import { ScrollTrigger } from 'gsap/ScrollTrigger'

const { Layout } = DefaultTheme
const router = useRouter()

gsap.registerPlugin(ScrollTrigger)

let tweens = []
let scrollTriggers = []

function killAll() {
  tweens.forEach(t => t.kill())
  tweens = []
  scrollTriggers.forEach(t => t.kill())
  scrollTriggers = []
}

// 首页 Hero 动画
function animateHero() {
  const hero = document.querySelector('.VPHero')
  if (!hero) return

  const name = hero.querySelector('.name')
  const text = hero.querySelector('.text')
  const tagline = hero.querySelector('.tagline')
  const buttons = hero.querySelectorAll('.actions .VPButton')

  // 先设置初始状态
  if (name) gsap.set(name, { y: 50, autoAlpha: 0 })
  if (text) gsap.set(text, { y: 30, autoAlpha: 0 })
  if (tagline) gsap.set(tagline, { y: 20, autoAlpha: 0 })
  if (buttons.length) gsap.set(buttons, { y: 20, autoAlpha: 0 })

  // 再动画到最终状态
  const tl = gsap.timeline({ defaults: { ease: 'power3.out' } })

  if (name) tl.to(name, { y: 0, autoAlpha: 1, duration: 1, ease: 'power4.out' })
  if (text) tl.to(text, { y: 0, autoAlpha: 1, duration: 0.8 }, '-=0.5')
  if (tagline) tl.to(tagline, { y: 0, autoAlpha: 1, duration: 0.8 }, '-=0.4')
  if (buttons.length) tl.to(buttons, { y: 0, autoAlpha: 1, stagger: 0.15, duration: 0.6 }, '-=0.3')

  tweens.push(tl)
}

// Features 卡片滚动动画
function animateFeatures() {
  const features = document.querySelectorAll('.VPFeature')
  if (!features.length) return

  // 设置初始状态
  gsap.set(features, { y: 60, autoAlpha: 0 })

  features.forEach((feature, i) => {
    const st = ScrollTrigger.create({
      trigger: feature,
      start: 'top 85%',
      onEnter: () => {
        const t = gsap.to(feature, {
          y: 0,
          autoAlpha: 1,
          duration: 0.8,
          delay: i * 0.1,
          ease: 'power2.out'
        })
        tweens.push(t)
      },
      once: true
    })
    scrollTriggers.push(st)
  })
}

// 文章页面动画 — 只对标题和段落级元素做轻量入场
function animateDoc() {
  const doc = document.querySelector('.VPDoc')
  if (!doc) return

  // 标题快速入场
  const h1 = doc.querySelector('h1')
  if (h1) {
    gsap.set(h1, { y: 20, autoAlpha: 0 })
    tweens.push(gsap.to(h1, { y: 0, autoAlpha: 1, duration: 0.5, ease: 'power2.out' }))
  }

  // 子标题滚动入场
  const subHeadings = doc.querySelectorAll('.main h2, .main h3')
  subHeadings.forEach((el) => {
    gsap.set(el, { y: 20, autoAlpha: 0 })
    const st = ScrollTrigger.create({
      trigger: el,
      start: 'top 90%',
      onEnter: () => {
        tweens.push(gsap.to(el, { y: 0, autoAlpha: 1, duration: 0.5, ease: 'power2.out' }))
      },
      once: true
    })
    scrollTriggers.push(st)
  })
}

function runAnimations() {
  killAll()
  // 延迟 300ms 确保 VitePress 渲染完成
  setTimeout(() => {
    animateHero()
    animateFeatures()
    animateDoc()
    ScrollTrigger.refresh()
  }, 300)
}

onMounted(() => {
  // 尊重用户减少动画偏好
  if (window.matchMedia('(prefers-reduced-motion: reduce)').matches) return

  runAnimations()
  router.onAfterRouteChange = () => runAnimations()
})

onUnmounted(() => {
  killAll()
})
</script>

<template>
  <Layout />
</template>
